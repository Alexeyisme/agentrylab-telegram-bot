...
class PresetService:
    ...
    def get_preset_categories(self, preset_info: Dict[str, Dict[str, Any]]) -> Dict[str, List[str]]:
        """
    ...
    def get_preset_display_info(self, preset_id: str, preset_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        return {
            'id': preset_id,
            'display_name': preset_info.get('display_name', preset_id.replace('_', ' ').title()),
            'description': preset_info.get('description', DefaultValues.DEFAULT_PRESET_DESCRIPTION),
            'emoji': preset_info.get('emoji', DefaultValues.DEFAULT_PRESET_EMOJI),
            'category': preset_info.get('category', DefaultValues.DEFAULT_PRESET_CATEGORY),
            'examples': preset_info.get('examples', [DefaultValues.DEFAULT_EXAMPLE_TOPIC])
        }
    
    def get_preset_examples_display(self, preset_id: str, preset_info: Dict[str, Any], max_examples: int = 5) -> List[str]:
        """
    ...
    def get_preset_summary(self, preset_info: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'presets_by_category': {cat: len(presets) for cat, presets in categories.items()},
            'most_common_category': max(categories.keys(), key=lambda k: len(categories[k])) if categories else None
        }
    
    def search_presets(self, query: str, preset_info: Dict[str, Dict[str, Any]]) -> List[str]:
        """
    ...
    def get_preset_recommendations(self, user_preferences: Dict[str, Any], preset_info: Dict[str, Dict[str, Any]]) -> List[str]:
        """
    ...
    def get_preset_statistics(self, preset_info: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        # Count examples
        total_examples = sum(len(info.get('examples', [])) for info in preset_info.values())
        ...
        return {
            ...
            'most_common_emoji': max(emoji_counts.keys(), key=lambda k: emoji_counts[k]) if emoji_counts else '🤖',
            'emoji_distribution': emoji_counts,
            'category_distribution': {cat: len(presets) for cat, presets in categories.items()}
        }
    
    def format_preset_list(self, preset_info: Dict[str, Dict[str, Any]], max_per_category: int = 10) -> str:
        """
        ...
        for category, preset_ids in categories.items():
            ...
            if len(preset_ids) > max_per_category:
                formatted_list += f"  ... and {len(preset_ids) - max_per_category} more\n"
