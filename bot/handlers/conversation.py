...
async def handle_topic_input(
    ...
    if not validation_result["valid"]:
        error_message = MessageTemplates.topic_validation_error(
            validation_result.get("error_type", "general")
        )
...
async def handle_conversation_input(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """
    ...
    if user_state.state != ConversationState.WAITING_FOR_USER_INPUT:
        await message.reply_text(
            MessageTemplates.waiting_for_turn_message(), parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        if success:
            ...
            await message.reply_text(
                MessageTemplates.message_sent_message(), parse_mode="Markdown"
            )
        else:
            await message.reply_text(
                MessageTemplates.waiting_for_turn_message(), parse_mode="Markdown"
        ...
        await message.reply_text("❌ Error sending your message. Please try again.")


@handle_errors("Error starting conversation. Please try again.")
async def start_conversation_with_agentrylab(
    update: Update, context: ContextTypes.DEFAULT_TYPE, preset_id: str, topic: str
) -> Optional[str]:
    """
    ...
    conversation_id = await conversation_service.start_conversation(
        user_id=user_id, preset_id=preset_id, topic=topic, max_rounds=10
    )
...
async def stream_conversation_events(
    update: Update, context: ContextTypes.DEFAULT_TYPE, conversation_id: str
) -> None:
    """
    ...
    try:
        ...
        async def handle_event(
            ...
            if event_type == "conversation_started":
                ...
                message = MessageTemplates.agent_message(
                    role or "agent", content, agent_id
                )
                ...
                if user_id:
                    state_manager.set_user_state(
                        user_id, ConversationState.WAITING_FOR_USER_INPUT
                    )
                ...
                if user_id:
                    state_manager.set_user_state(
                        user_id, ConversationState.CONVERSATION_ENDED
                    )
        ...
        try:
            ...
            if message_obj:
                await message_obj.reply_text(
                    MessageTemplates.connection_error_message(), parse_mode="Markdown"
                )
        except Exception as e2:
...
async def pause_conversation(
    ...
    # Show confirmation using template
    await message.reply_text(
        MessageTemplates.conversation_paused_message(), parse_mode="Markdown"
    )
...
async def resume_conversation(
    ...
    # Show confirmation using template
    await message.reply_text(
        MessageTemplates.conversation_resumed_message(), parse_mode="Markdown"
    )
...
async def stop_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Show confirmation using template
    await message.reply_text(
        MessageTemplates.conversation_stopped_message(), parse_mode="Markdown"
    )
