...
def create_preset_selection_keyboard(
    presets: List[str], preset_info: Dict[str, Dict[str, Any]]
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"📁 {category}", callback_data="category_header"
                    )
                ]
            )
        ...
        for i in range(0, len(preset_list), 2):
            ...
            for j in range(2):
                if i + j < len(preset_list):
                    ...
                    row.append(
                        InlineKeyboardButton(
                            button_text, callback_data=f"preset_{preset_id}"
                        )
                    )
            keyboard.append(row)
...
def create_preset_info_keyboard(preset_id: str) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "✅ Select This Preset", callback_data=f"select_{preset_id}"
            ),
            InlineKeyboardButton(
                "📝 See Examples", callback_data=f"examples_{preset_id}"
            ),
        ],
        [
...
def create_preset_examples_keyboard(preset_id: str) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "✅ Use This Example", callback_data=f"example_{preset_id}"
            ),
            InlineKeyboardButton(
                "✏️ Enter Custom Topic", callback_data=f"custom_{preset_id}"
            ),
        ],
        [
            InlineKeyboardButton(
                "🔙 Back to Preset Info", callback_data=f"info_{preset_id}"
            ),
            InlineKeyboardButton("❌ Cancel", callback_data="cancel"),
        ],
    ]
...
def create_topic_confirmation_keyboard(
    preset_id: str, topic: str
) -> InlineKeyboardMarkup:
    """
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "✅ Start Conversation", callback_data=f"start_{preset_id}"
            ),
...
def get_preset_description(preset_id: str) -> str:
    ...
    return descriptions.get(
        preset_id.lower(), "A multi-agent conversation with AI agents."
    )
